...
@section('content')
    ...
    <h1 class="text-2xl border-l-8 border-red-600 pl-3 uppercase m-4">Search Results For "{{ $query }}"</h1>
    ...
    @if($books->count())
                    ...
                    <ul class="w-80 pc:h-665 mobile:h-600 overflow-hidden bg-white shadow-lg p-4 rounded-lg relative">
                        ...
                        <li class="flex justify-center items-center gap-2">
                            @foreach (explode(',', $book->genres) as $genre)
                                <span class="odd:bg-red-400 even:bg-blue-500 py-1 px-2 text-sm text-white rounded-lg">{{ $genre }}</span>
                            @endforeach
                        </li>
        ...
        {{ $books->links() }}
    @else
        <p class="bg-red-300 text-red-700 p-3 rounded-lg mt-6">Sorry could'nt find a match</p>
    @endif
